    ...
    while pending:
        ...
        if isinstance(node, dict):
            for key, value in node.items():
                if key in {"$ref", "$dynamicRef"} and isinstance(value, str) and value and not value.startswith("#"):
...
def _structured_output_contract(value: Any, *, responses: bool) -> Optional[Dict[str, Any]]:
    """Normalize and compile Chat ``response_format`` or Responses ``text.format``."""
    if value is None:
        return None
    ...
    name, schema, strict = schema_spec.get("name"), schema_spec.get("schema"), schema_spec.get("strict", False)
...
def _validated_structured_output(contract: Optional[Dict[str, Any]], text: str) -> str:
    ...
    try:
        ...
        raise StructuredOutputValidationError("model output was not valid finite JSON") from exc
    ...
    if contract["type"] == "json_schema":
        try:
            jsonschema.Draft202012Validator(contract["schema"], registry=_NO_RETRIEVAL_SCHEMA_REGISTRY).validate(parsed)
...
def _hermes_sidecar(
    *, session_id: Optional[str], continuation: str, route: str,
    ...
    structured_output_capable: bool = False,
) -> Dict[str, Any]:
    ...
    return {
        ...
        "output_contract": {
            ...
            "route_capable": bool(structured_output_capable),
            "validated": validated,
        },
        "route": route,
...
def _failure_sidecar_context(context: Dict[str, Any], error: BaseException) -> Dict[str, Any]:
    merged = dict(context)
    result = getattr(error, "result", None)
    if isinstance(result, dict) and "_structured_output_capable" in result:
        merged["structured_output_capable"] = bool(result["_structured_output_capable"])
    return merged


def _response_output_message(
    text: str, *, status: str, message_id: Optional[str] = None,
) -> Dict[str, Any]:
    """Build one complete OpenAI Responses assistant output-message item."""
    return {
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    async def _handle_caller_inference(self, request):
        """Stateless caller-owned tool inference, separate from the agent handlers."""
    ...
    def _create_agent(
        ...
        defer_persistence: bool = False) -> Any:
        ...
        agent_kwargs = {
            ...
            "fallback_model": (
                None
                if confirmed_runtime_lock or format_only or output_contract
        ...
        try:
            ...
            if not str(exc).startswith("No LLM provider configured."):
                raise
            raise _ProviderAuthResolutionError("Provider authentication failed") from exc
        if output_contract:
            from agent.models_dev import get_model_info
            from agent.structured_output import unsupported_reason

            api_mode = getattr(agent, "api_mode", None)
            reason = unsupported_reason(output_contract["provider_format"], api_mode)
            if reason:
                raise StructuredOutputRequestError(reason)
            info = get_model_info(
                str(getattr(agent, "provider", "") or ""),
                str(getattr(agent, "model", "") or ""),
                allow_network=False,
            )
            capable = bool(info and info.structured_output)
            if output_contract["type"] == "json_schema" and output_contract.get("strict") and not capable:
                raise StructuredOutputRequestError(
                    "The effective model does not advertise strict structured-output support."
                )
            agent._structured_output_capable = capable
            agent._gateway_response_format = output_contract["provider_format"]
            overrides = dict(getattr(agent, "request_overrides", {}) or {})
            if api_mode == "codex_responses":
                overrides["text"] = {"format": output_contract["responses_format"]}
            elif api_mode == "chat_completions":
                overrides["response_format"] = output_contract["provider_format"]
    ...
    async def _run_agent_with_structured_output_repair(
        ...
        *,
        ...
        if provider_auth_error:
            raise _ProviderAuthResolutionError(_redact_api_error_text(provider_auth_error), result=result)
        ...
        try:
            ...
            repair_provider_auth_error = (
                repair_result.get("_provider_auth_error") if isinstance(repair_result, dict) else None
            )
            if repair_provider_auth_error:
                raise _ProviderAuthResolutionError(
                    _redact_api_error_text(repair_provider_auth_error), result=repair_result
                )
            ...
            try:
                ...
                raise StructuredOutputValidationError(str(exc), result=initial_result) from exc
            usage = {
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        try:
            output_contract = _structured_output_contract(
                body.get("response_format"), responses=False
            )
        except ValueError as exc:
            return web.json_response(
                _openai_error(str(exc), code="invalid_response_format"), status=400
            )
        sidecar_context = {
            "continuation": "session" if provided_session_id else ("caller_history" if history else "new"),
            "route": model_name if route else "default",
        ...
        if idempotency_key:
            ...
            except StructuredOutputRequestError as exc:
                return web.json_response(
        ...
        sidecar_context["structured_output_capable"] = bool(
    ...
    async def _write_sse_responses(
        ...
        try:
            ...
            try:
                result, agent_usage = await agent_task
                usage = agent_usage or usage
                if sidecar_context is not None and isinstance(result, dict):
                    sidecar_context["structured_output_capable"] = bool(
                        result.get("_structured_output_capable")
                    )
            ...
            if agent_error:
                ...
                failed_env["error"] = {
                    ...
                    "code": "structured_output_validation_failed" if structured_output_failed else "agent_error",
                }
                failed_env["usage"] = {
                    "input_tokens": usage.get("input_tokens", 0),
                    "output_tokens": usage.get("output_tokens", 0),
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            output_contract = _structured_output_contract(
                text_config.get("format") if isinstance(text_config, dict) else text_config,
                responses=True,
            )
        except ValueError as exc:
            return web.json_response(
        ...
        sidecar_context = {
            "continuation": "conversation" if conversation else (
                "caller_history" if raw_history else (
                    "previous_response_id" if previous_response_id else "new"
                )
            ),
        ...
        if idempotency_key:
            ...
            try:
                ...
                return _failed_response(
                    ...
                    status=500,
                    code="agent_error",
                    validated=not bool(output_contract),
                )
        else:
            try:
    ...
    async def _run_agent(
        ...
        output_contract: Optional[Dict[str, Any]] = None, format_only: bool = False,
        defer_persistence: bool = False) -> tuple:
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    agent = self._create_agent(
                        ...
                        session_model=session_model, confirmed_runtime_lock=confirmed_runtime_lock,
                        output_contract=output_contract, format_only=format_only,
                    ...
                    if isinstance(result, dict):
                        result["_structured_output_capable"] = bool(
                            getattr(agent, "_structured_output_capable", False)
                        )
                    ...
                    return (
                        {"final_response": "⚠️ Provider authentication failed",
                         "_provider_auth_error": "Provider authentication failed", "messages": [],
                         "api_calls": 0, "tools": []},
